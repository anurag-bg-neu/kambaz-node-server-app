...
let todos1 = [{id:555, completed: false}];
...
export default function WorkingWithArrays(app) {
  ...
  const getTodos1 = (req, res) => {
    const { completed } = req.query;
    if (completed !== undefined) {
      const completedBool = completed === "true";
      const completedTodos = todos1.filter((t) => t.completed === completedBool);
      res.json(completedTodos);
      return;
    }
    res.json(todos1);
  ...
  const createNewTodo = (req, res) => {
    ...
    const newTodo = {
      ...
      title: "New Task",
      completed: false,
      description: `New Description for ${thisId}`
    };
    todos.push(newTodo);
    res.json(todos);
  };
